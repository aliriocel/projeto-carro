...
export class CarroService {
  ...
  cadastrar(obj: any): Observable<any> {
    const observable =
      from(this.firestore.collection('carro').add(obj));
    return observable;
  }
  ...
  atualizar(id: string, dados: any): Observable<any> {
    const observable =
      from(this.firestore.collection('carro').doc(id).set(dados));
    return observable;
  }

  excluir(id: string): Observable<any> {
    const observable =
      from(this.firestore.collection('carro').doc(`${id}`).delete());
    return observable;
  }
  ...
  uploadFoto(modelo): Observable<any> {

    let fotoBlob = this.util.dataUriToBlob(this.fotoBlob);
    let observable = from(
      this.fireStorage.storage.ref().child(`/carro/${modelo}.jpg`).put(fotoBlob));
    return observable;
  }
